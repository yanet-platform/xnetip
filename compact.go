...
type Network interface {
	IPv4Network | IPv6Network | IPNetwork
}
...
func (m Compact[T]) String() string {
	...
	// The constraint leaves IPNetwork as the only remaining
	// instantiation, and it renders as the family it holds.
	network := any(m.Network).(IPNetwork)
	var buffer [91]byte
...
func appendCompact4(b []byte, network IPv4Network) []byte {
...
// bare address of a host route, the network's own text otherwise.
func appendCompact6(b []byte, network IPv6Network) []byte {
...
// otherwise. An IPNetwork is written in its own family, so the
