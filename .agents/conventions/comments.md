...
- **Detailed**: up to 6–8 lines, soft — the ceiling is a guideline, not a gate. Preconditions, failure modes, the correctness argument, links to the Rust reference (`../netip/src/net.rs:1200`) when the argument lives there. Still prose, not a bulleted spec. If it grows past the ceiling, the comment is documenting a function that should be split.
...
- **Self-contained**: never reference labels the reader cannot see (theorem names from plan files, PR numbers, Rust line numbers without the path). Rust references are written as `../netip/src/net.rs:NNNN` and are allowed only in the detailed block.
