...
// function is the exact counterpart of [Network4.ToContiguous],
// [Network6.ToContiguous], and [Network.ToContiguous].
...
// ContiguousFromCIDR4 returns the block of addr with the prefix bits
// preserved and host bits cleared.
//
// The mask built from a length is a leading run of ones, so the
// result is a CIDR block by construction. The address must satisfy
// [netip.Addr.Is4]. An IPv6 address, IPv4-mapped included, or the invalid zero
...
// ContiguousFromCIDR6 returns the block of addr with the prefix bits
// preserved and host bits cleared.
//
// The mask built from a length is a leading run of ones, so the
// result is a CIDR block by construction. The address must satisfy
...
// silently. An
// address satisfying [netip.Addr.Is4] or the invalid zero [netip.Addr] is
// rejected with [ErrAddrFamilyMismatch] — and bits must be in the range 0
// through 128, otherwise [ErrCIDROverflow] is returned.
...
// ContiguousFromCIDR returns the block of addr with the prefix bits
...
// IPv6 and stays IPv6, following [net/netip] family semantics. The invalid
...
//
// ok is false when the prefix is invalid or its address does not satisfy
// [netip.Addr.Is4]. An IPv4-mapped prefix is IPv6, so convert it through
// [ContiguousFromPrefix6] instead. A valid prefix always carries a
// contiguous mask, so the conversion accepts every valid IPv4 prefix
// and is the exact inverse of [Contiguous.Prefix]. The round trip through
// either direction is the identity on masked prefixes.
...
// block, host bits cleared.
//
// ok is false when the prefix is invalid or its address satisfies
// [netip.Addr.Is4]. An IPv4-mapped IPv6 prefix is IPv6 and is accepted. A valid prefix
// always carries a contiguous mask, so the conversion accepts every
// valid IPv6 prefix and is the exact inverse of [Contiguous.Prefix]. The
// round trip through either direction is the identity on masked prefixes.
...
//
// A prefix whose address satisfies [netip.Addr.Is4] becomes an IPv4 block.
// Anything else, including an IPv4-mapped IPv6 prefix, becomes an IPv6
// block, following [net/netip] semantics. ok is false only for the invalid
// zero prefix. A valid prefix always
// carries a contiguous mask, so the conversion is the exact inverse
// of [Contiguous.Prefix], and the round trip through either direction is
// the identity on masked prefixes.
...
// are, and safe to copy. The wrapper is generic over the family. A
// [Contiguous] instantiated with [Network4] is the IPv4 CIDR block, one
// instantiated with [Network6] is the IPv6 block, and one instantiated
// with [Network] is family-agnostic.
...
// wrapped networks' containment. Blocks of different families in the
// [Network] instantiation never contain each other.
...
//
// The answer follows [netip.Prefix.Contains]: the operation is total,
// with cross-family, zoned, and invalid addresses not contained. For a
// [Network] instantiation the address must be of the block's own family.
...
// networks' lowest-mask-bit merge. On ok=false the first result is
// the zero block.
...
// disjointness match the wrapped networks' set difference. The
...
// family: an IPv4 block of the [Network] instantiation yields a prefix
// whose address satisfies [netip.Addr.Is4], never the mapped storage form.
...
// family. The output parses back with [ParseContiguous4],
// [ParseContiguous6], or [ParseContiguous], according to the
// instantiation.
...
// mask. The output parses back with [ParseContiguous4],
// [ParseContiguous6], or [ParseContiguous], according to the
// instantiation.
...
//
// The text is the [Contiguous.String] form of the block: an address,
// "/", and a prefix length. It never fails and allocates only the
// returned slice.
...
//
// The text must be accepted by [ParseContiguous4], [ParseContiguous6],
// or [ParseContiguous], according to the instantiation. A valid network
// with a non-contiguous mask wraps [ErrNonContiguousMask]. Empty text
// wraps [ErrEmptyInput] because the zero wrapper is a valid block. The
// receiver is untouched on any error.
...
// generic method call self-typed binary operations on the wrapped
// value. Per-family signatures never reach a plain union's method set.
// It is a constraint for type parameters, not an interface for values.
// Methods are added by the wrapper session that first needs them.
type network[T any] interface {
	...
	// parseText is the concrete type's route to its family-specific
	// contiguous parser, called on a zero value as a dispatch token.
